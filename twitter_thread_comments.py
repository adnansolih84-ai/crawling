#!/usr/bin/env python3
"""Crawl Twitter/X thread comments from a sorted tweet CSV.

This script loads an existing tweet CSV, sorts by `retweet_count` (or
`created_at` if no retweet count is available), extracts unique tweet URLs,
then crawls thread comments for each tweet URL using `tweet-harvest`.

Usage:
  export TWITTER_AUTH_TOKEN=...
  export INPUT_CSV_PATH=/path/to/input.csv
  export OUTPUT_DIR=thread-comments
  export GDRIVE_DIR=/path/to/drive/folder  # optional
  python3 twitter_thread_comments.py

Optional environment variables:
  TWEET_LIMIT     Number of comments per thread crawl (default: 15000)
  MAX_THREADS     Maximum number of thread URLs to crawl (default: all)
"""
...
def main() -> int:
    ...
    for idx, url in enumerate(tweet_urls, start=1):
        ...
        output_file = os.path.join(temp_dir, build_filename("thread_comments", idx))

        command = (
            f'npx --yes tweet-harvest@latest -o {shlex.quote(output_file)} '
